    ...
    <div>
        <div class="mb-3">
            ...
            <select name="table" class="form-select" id="InputCategory">
                <option selected value="Guitars">Гітара</option>
                <option value="Keyss">Клавішні</option>
                <option value="Drums">Ударні</option>
                <option value="Winds">Духові</option>
                <option value="Bowed">Смичкові</option>
...
<script>
    const subCategories = {
        Guitars: <?= json_encode(\models\Guitars::findCategory()) ?>,
        Keyss: <?= json_encode(\models\Keyss::findCategory()) ?>,
        Drums: <?= json_encode(\models\Drums::findCategory()) ?>,
        Winds: <?= json_encode(\models\Winds::findCategory()) ?>,
        Bowed: <?= json_encode(\models\Bowed::findCategory()) ?>,
        Audio: <?= json_encode(\models\Audio::findCategory()) ?>
    };
